...
class DatabaseManager:
    ...
    def add_book(self, book: Book) -> int:
        ...
        self.cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (book.title, book.author, book.isbn, book.year, book.quantity, book.quantity))
        self.connection.commit()
    ...
    def update_book(self, book_id: int, **kwargs: Any) -> bool:
    ...
    def update_reader(self, reader_id: int, **kwargs: Any) -> bool:
        ...
        self.cursor.execute(
            f"UPDATE readers SET {set_clause} WHERE id = ?",
            values
        )
        self.connection.commit()
        return self.cursor.rowcount > 0
    ...
    def update_loan(self, loan_id: int, **kwargs: Any) -> bool:
        """Обновляет данные выдачи"""
        if not kwargs:
            return False
    ...
    def get_reader_loans(self, reader_id: int) -> List[Loan]:
        """Получает выдачи конкретного читателя"""

...
class LoanView(ttk.Frame):
    def __init__(
        self, parent, loan_controller, book_controller, reader_controller
    ) -> None:
    ...
    def create_widgets(self) -> None:
        ...
        ttk.Button(btn_frame, text="Create Loan", command=self.create_loan).pack(
            side="left", padx=5
        )
        ttk.Button(
            btn_frame, text="Return Selected", command=self.return_selected
    ...
    def refresh_loans(self) -> None:
        ...
        if filter_status == "all":
            ...
            loans = [
                ...
                if not getattr(loan, "is_returned", False)
            ]
        ...
        def save():
            try:
                ...
                messagebox.showerror(
                    "Error", "Please enter valid data (dates format: YYYY-MM-DD)"
                )
                return
            ...
            loan_id = self.loan_controller.create_loan(
